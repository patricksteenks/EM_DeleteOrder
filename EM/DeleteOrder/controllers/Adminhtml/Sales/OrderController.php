            ...
            foreach ($orderIds as $orderId) {
					...
					if($order->increment_id){				
						...
						if(in_array($table_sales_flat_creditmemo_comment,$rsc_table)){
                            $query = "DELETE FROM $table_sales_flat_creditmemo_comment WHERE parent_id IN (SELECT entity_id FROM $table_sales_flat_creditmemo WHERE order_id = :orderId)";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);                            
						}
						
						if (in_array('sales_flat_creditmemo_item', $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_creditmemo_item WHERE parent_id IN (SELECT entity_id FROM $table_sales_flat_creditmemo WHERE order_id = :orderId)";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						
						if (in_array($table_sales_flat_creditmemo, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_creditmemo WHERE order_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						
						if (in_array($table_sales_flat_creditmemo_grid, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_creditmemo_grid WHERE order_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						
						if (in_array($table_sales_flat_invoice_comment, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_invoice_comment WHERE parent_id IN (SELECT entity_id FROM $table_sales_flat_invoice WHERE order_id = :orderId)";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						
						if (in_array($table_sales_flat_invoice_item, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_invoice_item WHERE parent_id IN (SELECT entity_id FROM $table_sales_flat_invoice WHERE order_id = :orderId)";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }
                        
                        if (in_array($table_sales_flat_invoice, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_invoice WHERE order_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						
						if (in_array($table_sales_flat_invoice_grid, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_invoice_grid WHERE order_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						...
						if (in_array($table_sales_flat_shipment_comment, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_shipment_comment WHERE parent_id IN (SELECT entity_id FROM $table_sales_flat_shipment WHERE order_id = :orderId)";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						
						if (in_array($table_sales_flat_shipment_item, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_shipment_item WHERE parent_id IN (SELECT entity_id FROM $table_sales_flat_shipment WHERE order_id = :orderId)";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }
                        
                        if (in_array($table_sales_flat_shipment_track, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_shipment_track WHERE order_id IN (SELECT entity_id FROM $table_sales_flat_shipment WHERE order_id = :orderId)";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						
						if (in_array($table_sales_flat_shipment, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_shipment WHERE order_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }
                        
                        if (in_array($table_sales_flat_shipment_grid, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_shipment_grid WHERE order_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						
						if (in_array($table_sales_flat_order, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_order WHERE entity_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }
                        
                        if (in_array($table_sales_flat_order_address, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_order_address WHERE parent_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }                        
						
						if (in_array($table_sales_flat_order_item, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_order_item WHERE order_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }
                        
                        if (in_array($table_sales_flat_order_payment, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_order_payment WHERE parent_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }
                        
						if (in_array($table_sales_flat_order_status_history, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_order_status_history WHERE parent_id = :orderId";
                            $params = [':orderId' => $orderId];
                            $write->query($query, $params);
                        }
                        
                        if ($incId && in_array($table_sales_flat_order_grid, $rsc_table)) {
                            $query = "DELETE FROM $table_sales_flat_order_grid WHERE increment_id = :incId";
                            $params = [':incId' => $incId];
                            $write->query($query, $params);
                        }                        
						
						$query = "SHOW TABLES LIKE :tableLogQuote";
                        $params = [':tableLogQuote' => '%' . $table_log_quote . '%'];
                        $rsc_table_l = $write->fetchCol($query, $params);

                        if ($quoteId && $rsc_table_l) {
                            $query = "DELETE FROM $table_log_quote WHERE quote_id = :quoteId";
